...
Covers:
  - Budget satisfaction: actual avg_bits ~ budget (within tolerance)
...
class TestBudgetSatisfaction:
    ...
    def test_budget_16_all_fp16(self):
        rd = _rd_table(16, 64)
        result = allocate(rd, 16.0, FORMATS, 64, align_groups=False)
        for j, fmt in result.assignments.items():
            assert fmt == "FP16"
    ...
    def test_budget_just_below_maximum(self):
        rd = _rd_table(16, 64)
        result = allocate(rd, 15.99, FORMATS, 64, align_groups=False)
...
class TestOptimality:
    ...
    @pytest.mark.parametrize("budget", [6.0, 8.0, 12.0, 16.0])
...
class TestMonotonicity:
    def test_distortion_decreases_with_budget(self):
        rd = _rd_table(16, 64)
        budgets = [5.0, 8.0, 10.0, 14.0, 16.0]
...
class TestPermutation:
    ...
    @pytest.mark.parametrize("budget", [5.0, 10.0, 16.0])
    ...
    def test_permutation_groups_by_format(self):
        rd = _rd_table(32, 64)
        result = allocate(rd, 10.0, FORMATS, 64, align_groups=False)
...
class TestAllocationResultFields:
    ...
    def test_avg_bits_nonnegative(self):
        rd = _rd_table(16, 64)
        result = allocate(rd, 10.0, FORMATS, 64)
    ...
    def test_format_stats_total_bits_consistent(self):
        ...
        result = allocate(rd, 10.0, FORMATS, n_in)
...
class TestAllocateLayer:
    def test_returns_allocation_result(self):
        ...
        result = allocate_layer(w, 10.0, align_groups=False)
...
class TestSweepBudgets:
    ...
    def test_all_results_are_allocation_result(self):
        w = _rand_weight(16, 64)
        results = sweep_budgets(w, [4.0, 10.0, 16.0], align_groups=False)
        for r in results:
    ...
    def test_extreme_budgets(self):
        w = _rand_weight(16, 64)
        results = sweep_budgets(w, [4.0, 16.0], align_groups=False)
...
def test_infer_n_elements_per_channel():
    rd = _rd_table(8, 64)
    result = allocate(rd, 10.0, FORMATS, align_groups=False)
    assert isinstance(result, AllocationResult)
...
class TestFormatSubset:
    def test_two_format_subset(self):
        rd = _rd_table(8, 64)
        r4 = allocate(rd, 4.0, ["NVFP4", "FP16"], 64, align_groups=False)
        r16 = allocate(rd, 16.0, ["NVFP4", "FP16"], 64, align_groups=False)
    ...
    def test_single_format(self):
        rd = _rd_table(8, 64)
        result = allocate(rd, 8.0, ["FP8"], 64, align_groups=False)
...
def test_determinism():
    rd = _rd_table(16, 64)
    r1 = allocate(rd, 10.0, FORMATS, 64, align_groups=False)
    r2 = allocate(rd, 10.0, FORMATS, 64, align_groups=False)
...
class TestGroupAlignment:
    ...
    def test_aligned_groups_are_multiples_of_128(self):
        ...
        result = allocate_layer(w, 10.0, align_groups=True)
    ...
    def test_align_groups_function_direct(self):
        ...
        assignments = {i: "NVFP4" for i in range(200)}
    ...
    def test_align_groups_no_change_when_already_aligned(self):
        ...
        assignments = {i: "NVFP4" for i in range(128)}
        ...
        for fmt in aligned.values():
            counts[fmt] = counts.get(fmt, 0) + 1
        assert counts.get("NVFP4", 0) == 128
        assert counts.get("FP16", 0) == 128
