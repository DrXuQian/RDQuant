...
def test_correctness():
    ...
    workspace_nvfp4_marlin = torch.zeros_like(workspace)
    tile_counters_nvfp4_marlin = torch.zeros_like(tile_counters)
    y_splitk_nvfp4_marlin = rdquant_cuda.fused_mixed_gemv_marlin_weights_splitk_nvfp4_marlin(
        x,
...
def bench_shapes(parallel_k_mode, warmup, repeat, sweep_warmup, sweep_repeat):
    ...
    for shape_idx, (name, N, K, N_fp4, N_fp8) in enumerate(QWEN3_4B_SHAPES):
        ...
        workspace_nvfp4_marlin = torch.zeros_like(workspace)
        tile_counters_nvfp4_marlin = torch.zeros_like(tile_counters)
        t_splitk_nvfp4_marlin = bench(
            lambda: rdquant_cuda.fused_mixed_gemv_marlin_weights_splitk_nvfp4_marlin(
                x_fp16, w_fp4_q, marlin_fp4_scales, marlin_global_scale,
                w_fp8_q, w_fp8_scales,
                fp4_word_offsets, fp4_slot_map, fp8_word_offsets,
                inv_perm, workspace_nvfp4_marlin, tile_counters_nvfp4_marlin,
                N_fp4, N_fp8, K, parallel_k
            ),
            warmup=warmup,
            repeat=repeat,
