...
On an NVIDIA RTX 5090, the fused kernel with CUDA Graphs delivers a \textbf{1.92$\times$} decode speedup over BF16 (4.18\,ms vs.\ 8.03\,ms per token).
\end{abstract}
...
\caption{End-to-end inference latency on Qwen3-4B (RTX 5090). CUDA Graph eliminates Python dispatch overhead, achieving 2.59$\times$ decode speedup and 1.25$\times$ prefill speedup over BF16.}
\label{fig:latency}
\end{figure}
...
Table~\ref{tab:latency} reports full-model decode latency on RTX 5090.

\begin{table}[t]
\centering
\caption{Decode inference latency for Qwen3-4B on RTX 5090 (CUDA Graph, $M$=1). Speedup is relative to BF16.}
\label{tab:latency}
\begin{tabular}{lcc}
\toprule
\textbf{Method} & \textbf{Latency (ms/tok)} & \textbf{Speedup} \\
\midrule
BF16 baseline & 8.03 & $1.00\times$ \\
RDQuant (2$\times$ Marlin) & 12.57 & $0.64\times$ \\
\textbf{RDQuant (fused GEMV)} & \textbf{4.18} & $\mathbf{1.92\times}$ \\
\bottomrule
\end{tabular}
\end{table}
...
With CUDA Graph capture, the fused kernel achieves 4.18\,ms per token versus 8.03\,ms for BF16---a \textbf{1.92$\times$ speedup} with $\sim$4$\times$ memory compression.

\paragraph{Prefill.}
For prefill ($M \gg 1$), the fused GEMV kernel is not applicable (it targets $M$=1).
Prefill uses the 2$\times$ Marlin GEMM fallback with CUDA Graph, achieving a modest 1.25$\times$ improvement.
A fused grouped GEMM kernel for $M > 1$ is left as future work.
...
\end{tabular}
\end{table}
The measured 1.92$\times$ decode speedup from the fused GEMV kernel is close to the theoretical 2.8$\times$ from the bandwidth model, with the gap attributable to non-GEMM overhead (attention, layer norms, embedding lookup) that accounts for $\sim$2.5\,ms of the 4.18\,ms total.
...
Prefill ($M \gg 1$) still uses two Marlin GEMM launches per layer, limiting speedup to 1.25$\times$.
...
\begin{enumerate}
    ...
    \item A custom fused mixed-precision GEMV kernel processes NVFP4 and FP8 channels in a single launch, delivering a $1.92\times$ decode speedup on RTX 5090 (4.18\,ms vs.\ 8.03\,ms per token), with a packed model size of 2.62\,GB (33\% of BF16).
